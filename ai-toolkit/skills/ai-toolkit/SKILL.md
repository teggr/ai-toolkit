---
name: ai-toolkit
description: Optional post-install manager for ai-toolkit-managed Copilot customizations: inspect what is installed, update installed bundles, and reconcile local files with toolkit sources.
user-invocable: true
argument-hint: Describe your goal (e.g. "what toolkit bundles are installed here?", "update the installed review bundle", "compare local toolkit files with upstream")
---
...
- Distinguish install roots clearly:
  ...
  - User-global: `~/.copilot`
- Default to management of already-installed toolkit bundles, skills, agents, and instructions.
...
4. Detect currently installed bundles by matching local files to remote bundle file sets.
5. Execute requested action:
   - inspect: report installed/partial/missing bundle state and notable drift
